...
static int is_valid_variable_char(char c, int pos) {
    if ( (pos && c >= '0' && c <= '9') ||
            c == '_' ||
            (c >= 'a' && c <= 'z') ||
            (c >= 'A' && c <= 'Z'))
...
*/
static char *expand_variables(const char *in) {
    ...
    while ((var=strchr(pos, '$'))) {
        ...
        *var='\0';
        ...
        if (! val)
            val="";
        slen=(int)strlen(ret)+(int)strlen(val);
        elen=(int)strlen(begin+npos);
