...
void btheadup_send(struct headup_priv *this, enum btheadup_cmds cmd, void *data) {
    ...
    double *dir, *hdop, *routespeed, *vehiclespeed;
...
// Navigation status
    ...
    if (nav) {
        if (navigation_get_attr(nav, attr_nav_status, &nav_attr, NULL) || update_all) {
            char *navimgname = nav_status_to_text(nav_attr.u.num);
...
// Next turn image name and destination data
    ...
    if (item) {
        ...
        if (this->navnextturnimgtype != item->type || update_all) {
    ...
    if (item) {
        ...
        if (item->type == type_nav_position) {
            ...
            if (item_attr_get(item, attr_street_name, &streetname_attr) || update_all) {
                ...
                if (street) {
                    if (!strcmp(this->streetsysname, "")) {
                        ...
                        strncat(this->streetname, " ", sizeof(this->streetname) - strlen(this->streetname));
                        strncat(this->streetname, street, sizeof(this->streetname) - strlen(this->streetname));
                    }
                }
            } else {
                if (strcmp(this->streetsysname, "")) {
            ...
            if (item_attr_get(item, attr_destination_length, &dest_length_attr)) {
                int len = dest_length_attr.u.num;
            ...
            if (item) {
                ...
                if (item_attr_get(item, attr_street_name_systematic, &streetnamesysnext_attr)) {
                    ...
                    strncpy(this->nextstreetsysname, street, sizeof(this->nextstreetsysname));
                } else {
                ...
                if (item_attr_get(item, attr_street_name, &streetnamenext_attr)) {
                    ...
                    if (street) {
                        if (!strcmp(this->nextstreetsysname, "")) {
                            strncpy(this->nextstreetname, street, sizeof(this->nextstreetname));
                        } else {
                            strncpy(this->nextstreetname, this->nextstreetsysname, sizeof(this->nextstreetname));
                            strncat(this->nextstreetname, " ",
                                    sizeof(this->nextstreetname) - strlen(this->nextstreetname));
                            strncat(this->nextstreetname, street, sizeof(this->nextstreetname));
                        }
                    }
                } else {
                    if (strcmp(this->nextstreetsysname, "")) {
                ...
                if (item_attr_get(item, attr_length, &next_length_attr)) {
                    ...
                    int len = next_length_attr.u.num;
                    char *distnew = format_distance(len, imperial);
        ...
        case 4:
            ...
            if (vehicle_get_attr(curr_vehicle, attr_position_sats_used, &position_sats_used_attr, NULL)) {
                ...
                if (position_sats_used_attr.u.num >= 3)
                    strength = position_sats_used_attr.u.num - 1;
                if (strength > 5)
                    strength = 5;
                if (strength > 3) {
