...
bool position_was_invalid = true;
...
struct headup_priv {
    ...
    int gpshdop;
    char streetname[30];
    char streetsysname[10];
    char nextstreetname[30];
    char nextstreetsysname[10];
    char destlen[8];
    char desttime[25];
    int navnextturnimgtype;
    char distance[8];
    double direction;
...
void findHostDevice(struct headup_priv*);
uint16_t crc16_compute(uint8_t const *, uint32_t, uint16_t const *);
static char* format_distance(double, int );
void btheadup_init_ble(struct headup_priv*);
void btheadup_destroy(struct headup_priv*);
void btheadup_connected(void*);
void btheadup_disconnected(void*);
...
void btheadup_send(struct headup_priv *this, enum btheadup_cmds cmd, void *data);


static int btheadup_set_attr(struct headup_priv *this, struct attr *attr) {
...
#pragma unused(this)
    ...
    int *imp, *num, *strength;
    char *name="", *time="";
    //struct attr *position_attr;

    dbg(lvl_debug, "btheadup_send");
    ...
    if (nus_handle > 0) {
        ...
        case BTHEADUP_NAVIMAGENAME:
            name = (char*) data;
            btdata[0] = BTHEADUP_NAVIMAGENAME;

            memcpy(&btdata[x], name, strlen(name) + 1);
            x += strlen(name) + 1;
            dbg(lvl_debug, "BTHEADUP_NAVIMAGENAME COMMAND CODE: %i, data: %s", cmd, name)
            ;
            break;
        ...
        case BTHEADUP_TIME:
            ...
            btdata[0] = BTHEADUP_TIME;

            memcpy(&btdata[x], name, strlen(name) + 1);
            x += strlen(name) + 1;
            dbg(lvl_debug, "BTHEADUP_TIME COMMAND CODE: %i, data: %s", cmd, name)
            ;
            break;
        ...
        if (status < 0) {
            ...
            this->callback = callback_new_1(callback_cast(findHostDevice), this);
 ...
 * @param   this the btheadup struct containing the state of the plugin
 *
 * @return  nothing
 *
 ...
 *
 */
static void btheadup_idle(struct headup_priv *this) {
    ...
    struct attr position_attr, maxspeed_attr, speed_attr, vehicle_attr, imperial_attr, direction_attr, destination_attr,
               ...
               streetnamesysnext_attr, next_length_attr, valid;
    struct vehicle *curr_vehicle = 0;
    int imperial = 0;
    double vehicledir;
    double dir;
    double *height;
...
// send data for compass, position, speed, next turn instruction, ETA, speedlimit, conditional speedlimit, street, next street, gps status via Nordic NUS to the display device
    ...
    if (navit_get_attr(this->nav, attr_imperial, &imperial_attr, NULL)) {
        ...
        if (imperial != this->imperial || update_all) {
            btheadup_send(this, BTHEADUP_IMPERIAL, &imperial);
    ...
    if (vehicle_get_attr(curr_vehicle, attr_position_valid, &valid, NULL)) {
        if(valid.u.num!=attr_position_valid_valid) {
            ...
            if(position_was_invalid) {
                update_all=true;
                position_was_invalid=false;
            }
        }
    }
    ...
    if(obd) {
        ...
        if(this->obdspeed != obdspeed || update_all) {
            btheadup_send(this, BTHEADUP_OBDSPEED, &obdspeed);
            this->obdspeed = obdspeed;
        }

        double obdoil=0.0;
        navit_get_obdoiltemp(this->nav, &obdoil);
        if(this->obdoil != obdoil || update_all) {
            btheadup_send(this, BTHEADUP_OBDOILTEMP, &obdoil);
            this->obdoil = obdoil;
        }

        double obdcoolant=0.0;
        navit_get_obdcoolanttemp(this->nav, &obdcoolant);
        if(this->obdcoolant != obdcoolant || update_all) {
            btheadup_send(this, BTHEADUP_OBDCOOLANTTEMP, &obdcoolant);
            this->obdcoolant = obdcoolant;
        }

        double obdvoltage=0.0;
        navit_get_obdvoltage(this->nav, &obdvoltage);
        if(this->obdvoltage != obdvoltage || update_all) {
            btheadup_send(this, BTHEADUP_OBDVOLTAGE, &obdvoltage);
            this->obdvoltage = obdvoltage;
        }

    } else {
        this->obdspeed = 0;
        btheadup_send(this, BTHEADUP_OBDSPEED, &this->obdspeed);
        this->obdoil = 0;
        btheadup_send(this, BTHEADUP_OBDOILTEMP, &this->obdoil);
        this->obdcoolant = 0;
        btheadup_send(this, BTHEADUP_OBDCOOLANTTEMP, &this->obdcoolant);
        this->obdvoltage = 0;
        btheadup_send(this, BTHEADUP_OBDVOLTAGE, &this->obdvoltage);
    }

//    // Adapt ETA to current timezone
//    struct tm text_tm;
...
// Coordinates
    if (vehicle_get_attr(curr_vehicle, attr_position_coord_geo, &position_attr, NULL)) {
        if (round(100000 * position_attr.u.coord_geo->lat) / 100000 != this->latitude
                || round(100000 * position_attr.u.coord_geo->lng) / 100000 != this->longitude || update_all) {
            btheadup_send(this, BTHEADUP_COORDINATESGEO, (void*) &position_attr);
...
// Direction for compass
    if (vehicle_get_attr(curr_vehicle, attr_position_direction, &direction_attr, NULL)) {
        ...
        if (vehicledir != this->direction || update_all) {
            btheadup_send(this, BTHEADUP_DIRECTION, (void*) &vehicledir);
...
// Distance to destination
    if (navit_get_attr(this->nav, attr_destination, &destination_attr, NULL)
        ...
        if (destination_attr.u.pcoord->x != 0 && destination_attr.u.pcoord->y != 0) {
            ...
            if (strcmp(distnew, this->distance) || update_all) {
                btheadup_send(this, BTHEADUP_DISTANCE, distnew);
            ...
            if (dir != this->handledirection || update_all) {
                btheadup_send(this, BTHEADUP_HANDLEDIRECTION, &dir);
...
// Navigation status
    ...
    if (nav) {
        if (navigation_get_attr(nav, attr_nav_status, &nav_attr, NULL) || update_all) {
            char *navimgname = nav_status_to_text((int)nav_attr.u.num);
            btheadup_send(this, BTHEADUP_NAVIMAGENAME, navimgname);
        }
    }
...
// Next turn image name and destination data
    ...
    if (item) {
        ...
        if (this->navnextturnimgtype != (int)item->type || update_all) {
            btheadup_send(this, BTHEADUP_NAVNEXTTURNIMAGE, &item->type);
    ...
    if (item) {
        ...
        if (item->type == type_nav_position) {
            ...
            if (item_attr_get(item, attr_street_name_systematic, &streetnamesys_attr) || update_all) {
            ...
            if (item_attr_get(item, attr_street_name, &streetname_attr) || update_all) {
                ...
                if (street) {
                    if (!strcmp(this->streetsysname, "")) {
                        ...
                        snprintf(this->streetname, sizeof this->streetname, "%s%s%s", this->streetsysname, " ", street);
                    }
            ...
            if (strcmp(this->streetname, savstreetname) || update_all) {
                dbg(lvl_debug, "street=%s", this->streetname);
                btheadup_send(this, BTHEADUP_STREETNAME, this->streetname);
            ...
            if (item_attr_get(item, attr_destination_time, &dest_time_attr)) {
                ...
                if (strcmp(stime, this->desttime) || update_all) {
                    btheadup_send(this, BTHEADUP_DESTTIME, stime);
            ...
            if (item_attr_get(item, attr_destination_length, &dest_length_attr)) {
                ...
                if (strcmp(distnew, this->destlen) || update_all) {
                    btheadup_send(this, BTHEADUP_DESTLENGTH, distnew);
            ...
            if (item) {
                ...
                if (item_attr_get(item, attr_street_name, &streetnamenext_attr)) {
                    ...
                    if (street) {
                        if (!strcmp(this->nextstreetsysname, "")) {
                            ...
                            snprintf(this->nextstreetname, sizeof this->nextstreetname, "%s%s%s", this->nextstreetsysname, " ", street);
                        }
                    }
                } else {
                    if (strcmp(this->nextstreetsysname, "")) {
                ...
                if (strcmp(this->nextstreetname, savnextstreetname) || update_all) {
                    dbg(lvl_debug, "nextstreet=%s", this->nextstreetname);
                    btheadup_send(this, BTHEADUP_NEXTSTREETNAME, this->nextstreetname);
                ...
                if (item_attr_get(item, attr_length, &next_length_attr)) {
                    ...
                    if (strcmp(distnew, (char*) this->nextmanlen) || update_all) {
                        btheadup_send(this, BTHEADUP_NEXTMANEUVLENGTH, distnew);
...
//                        dbg(lvl_debug, "hdop=%f", *position_hdop_attr.u.numd);
                        if (*position_hdop_attr.u.numd != this->gpshdop || update_all) {
                            btheadup_send(this, BTHEADUP_GPSHDOP, position_hdop_attr.u.numd);
    ...
    if (strength != this->gpssigstrength || update_all) {
        btheadup_send(this, BTHEADUP_GPSSIGNALSTRENGTH, &strength);
    ...
    if (vehicle_get_attr(curr_vehicle, attr_position_height, &posheight_attr, NULL)) {
        ...
        if ((int) *height != (int) this->gpsheight || update_all) {
            btheadup_send(this, BTHEADUP_GPSHEIGHT, height);
...
//        }
        ...
        if (routespeed != this->routespeed || update_all) {
            btheadup_send(this, BTHEADUP_ROUTESPEED, &routespeed);
        ...
        if (vehiclespeed != this->vehiclespeed || update_all) {
            btheadup_send(this, BTHEADUP_VEHICLESPEED, &vehiclespeed);
...
/**
 * @brief       Initialize the btheadup plugin
 * @param[in]   btheadup    - the btheadup struct containing the state of the plugin
 *              nav     - the navit object
 *
 * @return      nothing
 *
 * Initialize the btheadup plugin
 *
 */
static void btheadup_init(struct headup_priv *this) {
    btheadup_init_ble(this);
}
...
void findHostDevice(struct headup_priv *this) {
    int dev_id, sock;
    char addr[19] = { 0 };  // 7F:C1:D3:37:66:31 + \0

    if (nus_handle > 0) {
        event_remove_timeout(this->idle);
        this->callback = callback_new_1(callback_cast(btheadup_idle), this);
...
void findHostDevice(struct headup_priv *this) {
    ...
    if(this->deviceserial != NULL)
        bthbtcontroller_init(this, btheadup_disconnected, btheadup_connected, this->deviceserial);
}

void btheadup_connected(void* _this) {
    struct headup_priv * this = (struct headup_priv *) _this;
    nus_handle = 1;
    update_all=true;
    callback_list_call_attr_0(this->cbl, attr_bt_connected);
    navit_say(this->nav, _("Bluetooth Headup connected"));
}

void btheadup_disconnected(void* _this) {
    struct headup_priv * this = (struct headup_priv *) _this;
    if(nus_handle)
        navit_say(this->nav, _("Bluetooth Headup disconnected"));
    nus_handle = 0;
    callback_list_call_attr_0(this->cbl, attr_bt_connected);
    this->nextmanlen[0]=0;
    this->gpssigstrength=0;
    this->gpsheight=0.0;
    this->gpshdop=0;
    this->streetname[0]=0;
    this->streetsysname[0]=0;
    ...
    this->destlen[0]=0;
    this->desttime[0]=0;
    this->navnextturnimgtype=0;
    this->distance[0]=0;
    this->direction=0;
    this->handledirection=0;
    this->imperial=0;
    this->latitude=0;
    this->longitude=0;
    this->routespeed=0;
...
/**
 * @brief   Opens the serial port and saves state to the btheadup object
 * @param[in]   btheadup - the btheadup struct containing the state of the plugin
 *
 * @return  nothing
 *
 * Opens the serial port and saves state to the btheadup object
 *
 */
void btheadup_init_ble(struct headup_priv *this) {
...
#if !defined(__APPLE__)
void btheadup_destroy(struct headup_priv* ptr) {
...
void btheadup_destroy(struct headup_priv* ptr) {
#pragma unused(ptr)
// TODO: stop any active scan here
}

#endif

static int btheadup_get_attr(struct headup_priv *priv, enum attr_type type, struct attr *attr) {
#pragma unused(priv)
    if(type==attr_bt_connected) {
        attr->type = type;
...
struct headup_methods btheadup_methods = {
    btheadup_destroy,
    btheadup_set_attr,
    btheadup_get_attr,
};
...
/**
 * @brief   Creates the btheadup plugin and set some default properties
 ...
 * @return  nothing
 *
 * Creates the btheadup plugin and set some default properties
 *
 */
static struct headup_priv* btheadup_new(struct navit *nav, struct headup_methods *meth, struct callback_list *cbl,
                                        struct attr **attrs) {
...
//return NULL;
    ...
    ret->nav=nav;
    ret->cbl = cbl;
    ret->callback=callback_new_1(callback_cast(btheadup_init), ret);
    *meth = btheadup_methods;
    while (attrs && *attrs)
        btheadup_set_attr(ret, *attrs++);
    navit_add_callback(nav, callback_new_attr_1(callback_cast(btheadup_init), attr_graphics_ready, ret));
    return ret;
}
...
void plugin_init(void) {
    plugin_register_category_headup("btheadup", btheadup_new );
