...
void bthbtcontroller_send(unsigned char* data, int length) {
    ...
    [bthbtcontroller.discoveredPeripheral writeValue:d forCharacteristic:bthbtcontroller.characteristic type:
...
(NSDictionary *)advertisementData RSSI:(NSNumber *)RSSI {
    ...
    NSLog(@"BTHEADUP - Discovered %@ at %@dBm,  ID: %@", peripheral.name, RSSI, peripheral.identifier);
    ...
    if (@available(iOS 10, *)) {
        utf8Str = [[peripheral.identifier UUIDString] UTF8String];
    } else {
        // in iOS 9 we use the devicename
        utf8Str = [peripheral.name UTF8String];
    }
